    ...
    if(!this->ukloniArguments.empty()){
        ...
        for(const auto& arg : argumentator.getArguments()){
            if(arg == "s"){
                ...
                *(this->outStream)  << "nepravilno unesena komanda. unesi argument -p za pomoc.";
                return;
            }else{
                *(this->outStream)  << "argument \"" + arg + "\" nije prepoznat. unesi argument -p za pomoc.";
                return;
            }
        }
        ...
        if(res == -1){
            ...
            *(this->outStream)  << "greska pri uklanjanju. putanja je nevazeca.";
        }else if(res == -2){
            *(this->outStream)  << "direktorijum na putanji nije prazan. iskoristi -s da prisilis uklanjanje."; 
        }
    }else{
        *(this->outStream)  << "putanja nije prosledjena. unesi argument -p za pomoc."; 
    }
}
